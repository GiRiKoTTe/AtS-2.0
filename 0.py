...
quick_setup_target_percent = [50.0,52.5,55.0,57.5,60.0,62.5,65.0,67.5,70.0,72.5,75.0,77.5,80.0,82.5,85.0,87.5,90.0,92.5,95.0,97.5,100.0
]
#Main
quick_setup_main = "main"
quick_setup_main_max = 400
quick_setup_main_single_at_8 = 90
quick_setup_main_behavior_RIR_sets = 5
...
quick_setup_aux_behavior_RIR_sets = 5
...
def setup_main_behavior_RIR_adjust(w):
  if w == 1:
    return quick_setup_main_behavior_RIR_adjust
  elif w >= 2 and w<= 21:
    return setup_main_behavior_RIR_adjust(w-1)

def setup_main_reps_per_set (w):
    if setup_main_intensity[w-1] = quick_setup_main_rep_target[w-1]:
      return 
def setup_main_reps_per_set (w):
  count=1
  while 1<=count<=21:

      if setup_main_intensity[w - 1] == quick_setup_main_intensity[count - 1]:
          setup_main_reps_per_set = quick_setup_main_rep_target[count - 1]
          break
      else :
          count+=1
  return setup_main_reps_per_set

def setup_main_last_set_RIR (w):
  count=1
  while 1<=count<=21:

      if setup_main_intensity[w - 1] == quick_setup_main_last_set_RIR_target[count - 1]:
          setup_main_last_set_RIR = quick_setup_main_last_set_RIR_target[count - 1]
          break
      else :
          count+=1
  return setup_main_last_set_RIR
...
def setup_aux_reps_per_set (w):
  ...
  while 1<=count<=21:
      ...
      if setup_aux_intensity[w - 1] == quick_setup_aux_intensity[count - 1]:
          setup_aux_lift_reps = quick_setup_aux_rep_target[count - 1]
          break
      else :
          count+=1
  return setup_aux_lift_reps

def setup_aux_last_set_RIR (w):
  count=1
  while 1<=count<=21:

      if setup_aux_intensity[w - 1] == quick_setup_aux_last_set_RIR_target[count - 1]:
          setup_aux_rir = quick_setup_aux_last_set_RIR_target[count - 1]
          break
      else :
          count+=1
  return setup_aux_last_set_RIR
...
p_RIR_main_last_set_RIR_target = setup_main_last_set_RIR
p_RIR_main_set_goal = setup_main_sets
...
def p_RIR_auxTM_weight(w):
  if w == 1:
    if p_RIR_auxTM_last_set_RIR_target[w-1] == "":
      return quick_setup_RIR_aux_max
    else:
...
def p_RIR_aux_weight(w):
  return round(((p_RIR_auxTM_weight(w)) * (setup_RIR_aux_intensity)), quick_setup_RIR_aux_rounding)
p_RIR_aux_reps_per_set = setup_aux_reps_per_set
p_RIR_aux_last_set_RIR_target = setup_aux_last_set_RIR
p_RIR_aux_set_goal = setup_aux_sets
...
def table_main(w): 
  return [p_RIR_main_weight(1),p_RIR_main_reps_per_set(w),p_RIR_main_last_set_RIR_target(w),p_RIR_main_set_goal(w)]
def table_aux(w): 
  return [p_RIR_aux_weight(1),p_RIR_aux_reps_per_set(w),p_RIR_aux_last_set_RIR_target(w),p_RIR_aux_set_goal(w)]
#####################################################################################################################
print (p_RIR_main_reps_per_set(0))