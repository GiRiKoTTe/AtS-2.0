...
#Aux
quick_setup_aux = "aux"
quick_setup_aux_max = 420.0
quick_setup_aux_single_at_8 = 90.0
...
def untouched_aux_weight(w):
  return (x_round(((untouched_auxTM_weight(w))*(quick_setup_aux_intensity[w-1])),quick_setup_aux_rounding))/100
...
def p_RIR_auxTM_weight(w):
  if w == 1:
    ...
    if p_RIR_auxTM_last_set_RIR_target[w-1] == "":
      if p_RIR_aux_sets_completed[w-2] == "":
        return p_RIR_auxTM_weight(w-1)
...
def p_RIR_aux_weight(w):
  return (x_round(((p_RIR_auxTM_weight(w)) * (setup_aux_intensity[w-1])), quick_setup_aux_rounding))/100.0
...
p_RIR_aux_sets_completed = [""]*21
p_RIR_aux_RIR_on_last_set = [""]*21
